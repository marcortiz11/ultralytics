...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, Conv3D
from .transformer import TransformerBlock

__all__ = ('DFL', 'HGBlock', 'HGStem', 'SPP', 'SPPF', 'C1', 'C2', 'C3', 'C2f', 'C3x', 'C3TR', 'C3Ghost',
           'GhostBottleneck', 'Bottleneck', 'BottleneckCSP', 'Proto', 'RepC3', 'CARAFE', 'SPPF3D', 'C2f3D')
...
class DFL(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.conv(x.view(b, 4, self.c1, a).transpose(2, 1).softmax(1)).view(b, 4, a)
        return y
...
class SPPF3D(nn.Module):
    """Spatial Pyramid Pooling - Fast (SPPF3D) layer for YOLOv5 by Glenn Jocher."""

    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv3D(c1, c_, 1, 1, 0)
        self.cv2 = Conv3D(c_ * 4, c2, 1, 1, 0)
...
class C2f3D(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 3d-convolutions."""
...
class Bottleneck3D(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=((3, 3, 3), (3, 3, 3)), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
