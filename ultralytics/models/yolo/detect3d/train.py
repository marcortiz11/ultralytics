...
class DetectionTrainer(BaseTrainer):
    ...
    def build_dataset(self, img_path, mode='train', batch=None, motion=False, video=True, seq_length=12):
        """
        Build YOLO Dataset.
    ...
    def get_dataloader(self, dataset_path, batch_size=16, rank=0, mode='train', motion=False, video=False, seq_length=12):
        ...
        with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
            dataset = self.build_dataset(dataset_path, mode, batch_size, motion, video, seq_length)
        shuffle = mode == 'train'
